...
We do not report an EHR-contradiction rate. Our implementation of that
detector flags an answer when it contains a negation cue followed by a
term appearing on a diagnosis or laboratory line of the record, without
checking whether the negation is the record's own wording. Coded
diagnosis descriptions embed negation routinely---``spondylosis without
myelopathy'', ``type~2 diabetes without complications''---so an answer
that correctly reproduces a patient's coded diagnosis is scored as
contradicting it. We therefore withdraw the measure rather than report
a number we cannot support. The unsupported-claim rate is retained.

\begin{table*}[t]
\centering
\small
\setlength{\tabcolsep}{5pt}
\caption{Held-out evaluation, 300 questions, seven systems. Latency is mean end-to-end wall clock.}
\label{tab:main}
...
Our remaining hallucination measure is an automatic heuristic, not a
clinician-adjudicated annotation, and we perform no human evaluation
and no inter-annotator agreement study. As reported in
Section~\ref{sec:results}, we withdrew a second heuristic---an
EHR-contradiction detector---after establishing that it fires on
negation embedded in coded diagnosis descriptions rather than on
genuine contradiction; we report no contradiction rate in its place.
