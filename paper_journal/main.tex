% Elsevier elsarticle, numerical citation style, per the Journal of Biomedical
% Informatics Guide for Authors. Converted from IEEEtran (conference) --- see
% the conversion commit for the full list of substitutions.
%
% NOTE ON THE BIBLIOGRAPHY: this manuscript carries a hand-written
% `thebibliography` block, not BibTeX, so there was no \bibliographystyle to
% swap from IEEEtran.bst to elsarticle-num.bst. elsarticle renders a manual
% thebibliography as numbered citations, which is what JBI requires. Migrating
% the 18 entries to a .bib file + elsarticle-num.bst is a separate task and is
% deliberately NOT bundled into the class conversion.
...
% Overfull-hbox control. The intro and methods paragraphs contain long
% unbreakable tokens --- tied units like "6~GB", em-dash compounds, and
% \texttt{HAS\_SYMPTOM}-style identifiers --- which TeX cannot hyphenate and
% which therefore push past the margin at 12pt single column. emergencystretch
% gives it extra interword elasticity to resolve those lines instead of
% overflowing; raising \tolerance lets it accept the looser result.
\setlength{\emergencystretch}{3em}
\tolerance=1000
\hbadness=2000          % stop reporting the merely-loose lines this permits

% elsarticle sets \journal for the preprint line.
...
\author[inst1]{Daksh}
\ead{daksh6.mitblr2025@learner.manipal.edu}

\author[inst1]{Jayita Saha}
\ead{jayita.saha@manipal.edu}

\affiliation[inst1]{organization={Manipal Institute of Technology Bengaluru,
...
trade-off, not a metric artifact. Human validation of our hallucination
heuristics finds the unsupported-claim measure agrees with human
judgement ($\kappa = 0.749$) while an EHR-contradiction heuristic does
not ($\kappa = -0.037$, zero true positives); we withdraw the latter.
Repairing it shows contradiction concentrates in guideline-panel
questions the original could not detect (12 of 20 annotated rows versus
0 of 30 diagnosis rows), so rates from heuristics of this family are too
low rather than merely noisy. KG benefit also \textit{decreases}
...
\begin{keyword}
adaptive retrieval \sep retrieval-augmented generation \sep
knowledge graph \sep clinical question answering \sep
electronic health records \sep MIMIC-IV \sep QLoRA \sep
parameter-efficient fine-tuning \sep EHR sparsity \sep consumer GPU
\end{keyword}
...
BERTScore~F1~\cite{zhang2020} for quality; an unsupported-claim rate as
a heuristic hallucination proxy; and
latency, prompt tokens, and retrieved KG facts for cost. We originally
also reported an EHR-contradiction rate. Section~\ref{sec:halluc-validation}
reports a human validation of both hallucination heuristics: the
unsupported-claim rate is validated and retained, while the
EHR-contradiction rate is withdrawn as invalid and is not used anywhere
in our results. Confidence intervals are percentile bootstrap over
...
Table~\ref{tab:main} omits the EHR-contradiction rate. Human validation
(Section~\ref{sec:halluc-validation}) shows that heuristic agrees with
human judgement at $\kappa = -0.037$ with zero true positives, so we
withdraw it rather than report it. Its repaired successor, the
negation-contradiction rate, measures a strictly narrower phenomenon
and is validated only for part of the question mix; we therefore do not
substitute a corrected column here, and we report what the validation
does support in Section~\ref{sec:halluc-validation}. The
unsupported-claim rate is retained: it reaches $\kappa = 0.749$ against
human judgement on the same protocol.

\begin{table}[t]
\centering
\small
\setlength{\tabcolsep}{5pt}
\caption{Held-out evaluation, 300 questions, seven systems. T+E is the always-T+E policy, T+E+K the always-on hybrid, and Oracle the post-hoc upper bound. Lat.\ is mean end-to-end wall-clock latency and Tok.\ the mean prompt length in tokens. The EHR-contradiction rate reported in earlier versions of this work is withdrawn as invalid (Section~\ref{sec:halluc-validation}).}
\label{tab:main}
% Shrink only if the natural width still exceeds the text block; a table that
% already fits is left at full size rather than being scaled up.
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{%
\begin{tabular}{lrrrrrrr}
\toprule
System & BLEU & ROUGE-L & BERTScore~F1 & Unsupported & Lat.~(ms) & Tok. & KG facts \\
\midrule
T                      & 0.1588 & 0.2676 & 0.7937 & 0.5813 & 2209.6 & 1922 & 0.00 \\
T+E                    & 0.6859 & 0.7965 & 0.9553 & 0.3244 & 3341.3 & 2118 & 0.00 \\
T+E+K                  & 0.7481 & 0.8630 & \textbf{0.9708} & \textbf{0.1832} & 7527.7 & 2367 & 8.88 \\
\textbf{Router (ours)} & \textbf{0.7488} & \textbf{0.8632} & 0.9685 & 0.2554 & \textbf{4004.9} & 2130 & 2.04 \\
Random                 & 0.5131 & 0.6272 & 0.9002 & 0.3493 & 4297.6 & 2139 & 3.27 \\
StaticQType            & 0.7338 & 0.8392 & 0.9634 & 0.2696 & 3790.0 & 2101 & 1.34 \\
Oracle                 & 0.7497 & 0.8727 & 0.9708 & 0.2552 & 3961.7 & 2119 & 1.66 \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{summary_metrics.png}
...
We previously also reported an EHR-contradiction difference between the
router and always-on hybrid retrieval as statistically indistinguishable
($p = 0.157$). That comparison is withdrawn: human validation shows the
underlying detector has no measurable agreement with human judgement
(Section~\ref{sec:halluc-validation}), so the comparison related two
quantities that do not measure contradiction. The third H1 criterion
therefore rests on the unsupported-claim rate alone, which is the
component with human validation behind it, and it fails on that
component.
...
\input{sections/shap_explainability}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{sparsity_heatmap_bertscore_f1.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{router_mode_distribution.png}
\caption{Router mode selection overall and by EHR sparsity bucket.}
\label{fig:routing}
\end{figure}

\input{sections/negation_contradiction}

\input{sections/contraindication_violation}
...
\subsection{Implications for Automatic Hallucination Metrics}
...
The contradiction result in Section~\ref{sec:halluc-validation} makes a
sharper version of the same point. A pattern-matching detector can be
wrong in both directions at once, and neither failure is visible from
its output alone. Ours over-fired wherever the record's own vocabulary
contained negation---coded diagnoses embed it routinely---and
simultaneously missed most genuine contradictions, because the surface
form it matched was not the form in which contradictions were actually
expressed. Aggregate rates concealed both: the reported values sat in a
plausible range, were stable across systems, and were almost entirely
uninformative. We would not have discovered either failure without
adjudicating individual decisions against a human reference.

Two practices follow. First, a hallucination heuristic reported as a
headline safety number should carry an agreement statistic, since a
plausible-looking rate is not evidence that the detector fires on the
phenomenon it names. Second, when such a detector is repaired, the
repair should be validated on data disjoint from the cases that exposed
the defect; we found that a fix motivated by one question type did not
transfer to another, and would have overstated its own success had we
re-used the diagnostic sample.
...
Our hallucination measures are automatic heuristics. We validate them
against human judgement in Section~\ref{sec:halluc-validation}, but that
validation has its own limits. It is \emph{human--detector} agreement
from a single annotator, not inter-annotator agreement; the annotator was
blind to system identity and to the detector's output, but is an author
and is not blind to our hypotheses. No two-annotator agreement study was
performed, and no annotator holds a clinical qualification, so these
labels establish lexical correctness rather than clinical adjudication.

The repaired negation-contradiction measure captures only contradictions
expressed as \emph{negation of an asserted finding}. Answers that assert
something the record refutes---the complementary error---are not detected
by it or by any lexical rule, and we do not measure them. It is validated
for diagnosis questions and only partially for guideline-panel questions,
where its precision is $0.733$ (95\% CI $[0.449, 0.922]$) against a
pre-registered acceptance bar of $0.80$ that it does not meet. Its
residual false positives arise from a single answer template in which a
generic qualifier rather than a clinical finding is negated. We report
this rather than iterate further, because a correction derived from those
observations could not be tested on them.

Contradiction is undefined for text-only retrieval, which receives no
record to contradict. For systems that choose when to enter that mode
this exclusion is not neutral, and any per-system rate over such a metric
must be computed on a matched question set; we did not appreciate this in
our original analysis. Finally, we do not implement a KG-contradiction
measure, so we cannot say whether injected graph facts are themselves
respected.
